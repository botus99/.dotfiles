...
EMOJI=$(sed '1,/^exit/d' $0 | rofi -dmenu -i -l 10 -fn Symbola:size=20 -theme ${dir}/${theme}.rasi | grep -Po '^[^ ]*')
if [ "$EMOJI" ]; then
...
# curl -sL https://raw.githubusercontent.com/milesj/emojibase/master/packages/data/en/data.raw.json  | jq -r '.[] | .emoji + "  " + (.label | ascii_upcase) + "  " + (.tags | .? | map("#" + .) | join(" "))'
exit 1
...
🗑️  WASTEBASKET  #wastebasket #garbage #trash
🔒️  LOCKED  #closed
...
🚮  LITTER IN BIN SIGN  #garbage #litter #litter bin #trash
...
🚯  NO LITTERING  #forbidden #garbage #litter #no #not #prohibited #trash

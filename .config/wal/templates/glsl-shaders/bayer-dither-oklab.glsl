...
vec4 window_shader() {
    ...
    /* ------------------------------------------------------
       find two closest colors in palette (oklab)
    ---------------------------------------------------------
    ...
    for (int i = 0; i < 16; i++) {
        float dist = color_distance(pixel_lab, palette_lab[i], palette_len2[i]);
   ...
   //  ratio = smoothstep(0.0, 1.0, ratio);
    ...
    /* ------------------------------------------------------
       output brightness (clamp + invert)
    ------------------------------------------------------ */

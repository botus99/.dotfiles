...
#project_brutality_path="$HOME/.wads/Project Brutality Public Files/Project Brutality/project brutality 2.0.pk3"
project_brutality_path="$HOME/.wads/project-brutality/Project_Brutality_2024-10-05.pk3"
...
# SAVE FILE DIRECTORY
save_dir="$HOME/.config/gzdoom/savegames/project-brutality"


# custom config locataion
config_path="$HOME/.config/gzdoom/configs/project-brutality.ini"


# Check if required files exist
# ADD PATHS FROM ABOVE AS NEEDED
for path in "$iwad_path" "$project_brutality_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" "$cats_visor_base_path" "$cats_visor_path" "$dhtp_path"; do
    if [ ! -f "$path" ]; then

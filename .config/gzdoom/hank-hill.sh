...
# SAVE FILE DIRECTORY
save_dir="$HOME/.config/gzdoom/savegames/hank-hill"


# custom config locataion
config_path="$HOME/.config/gzdoom/configs/hank-hill.ini"


# Check if required files exist
# ADD PATHS FROM ABOVE AS NEEDED
for path in "$iwad_path" "$brutal_doom_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" "$cats_visor_base_path" "$cats_visor_path" "$dhtp_path"; do
    if [ ! -f "$path" ]; then
...
hank_path
#gzdoom -iwad "$iwad_path" -file "$brutal_doom_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" "$cats_visor_base_path" "$cats_visor_path" "$dhtp_path"  -savedir "$save_dir"
gzdoom -iwad "$iwad_path" -file "$hank_path"  -savedir "$save_dir"

...
#brutal_doom_path="$HOME/.wads/brutalv21/brutalv21.14.3_dev.pk3"
...
for path in "$iwad_path" "$brutal_doom_path"; do
    if [ ! -f "$path" ]; then
...
gzdoom -iwad "$iwad_path" -file "$going_down_path" "$brutal_doom_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" -savedir "$save_dir" -config "$config_path"

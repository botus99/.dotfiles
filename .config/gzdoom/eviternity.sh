...
# BRUTAL DOOM
brutal_doom_path="$HOME/.wads/brutalv21/brutalv21.15.1_dev.pk3"
...
# SAVE FILE DIRECTORY
save_dir="$HOME/.config/gzdoom/savegames/eviternity"

# custom config locataion
config_path="$HOME/.config/gzdoom/configs/eviternity.ini"

# Check if required files exist
# ADD PATHS FROM ABOVE AS NEEDED
for path in "$iwad_path" "$dhtp_path" "$beautiful_path"  "$brutal_doom_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" "$cats_visor_base_path" "$cats_visor_path"; do
    if [ ! -f "$path" ]; then
...
gzdoom -iwad "$iwad_path" -file "$dhtp_path" "$beautiful_path"  "$eviternity_path" "$tourretes_guy_path" "$rain_and_snow_path" "$cats_visor_base_path" "$cats_visor_path" -savedir "$save_dir"

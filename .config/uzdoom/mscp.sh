...
for path in "$iwad_path" "$dhtp_path" "$beautiful_path"  "$brutal_doom_path" "$doom_2016_music_path" "$tourretes_guy_path" "$rain_and_snow_path" "$mscp_path"; do
    if [ ! -f "$path" ]; then
...
ENABLE_VKBASALT=1 uzdoom -iwad "$iwad_path" -file "$dhtp_path" "$beautiful_path" "$rain_and_snow_path" "$mscp_path" -savedir "$save_dir" -config "$config_path"

...
for path in "$iwad_path" "$dhtp_path" "$beautiful_path" "$project_brutality_path" "$doom_2016_music_path" "$rain_and_snow_path"; do
    if [ ! -f "$path" ]; then
...
ENABLE_VKBASALT=1 uzdoom.appimage -iwad "$iwad_path" -file "$dhtp_path" "$beautiful_path" "$project_brutality_path" "$doom_2016_music_path" "$rain_and_snow_path" -savedir "$save_dir" -config "$config_path"
